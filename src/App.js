...
loadScript("https://cdn.bootcss.com/jquery/3.2.1/jquery.js", function() { 
     //加载,并执行回调函数
    //  alert('Load jquery successfully!')
});
...
// exec('python ../code/helloworld.py ',function(error,stdout,stderr){
//     if(stdout.length >1){
...
class App extends Component {
  ...
  beginSolve = () => {
    ...
    */
    ...
    for(var i=0;i<this.state.cubeDimension;i++)
    {
      F[i] = new Array(this.state.cubeDimension)
    }
    ...
    /**
      * B面，黄色
    */
   let B = new Array(this.state.cubeDimension)
   for(var i=0;i<this.state.cubeDimension;i++)
   {
     B[i] = new Array(this.state.cubeDimension)
   }
   for(let i =0; i<this.state.rubiksObject.length;i++)
    {
      if(this.state.cubes[i].position.y === this.state.cubeDimension-1)
      {
         var x = this.state.cubeDimension-1-this.state.cubes[i].position.x;
         var y = this.state.cubeDimension-1-this.state.cubes[i].position.z;
        //  console.log("pos x: ", x, "y: ", this.state.cubes[i].position.y, "z: ", y);
        //  console.log("color: ", this.colorJudge(this.state.cubes[i].material[2].color));
         
         B[y][x] = this.colorJudge(this.state.cubes[i].material[2].color);
      }
    }
    // console.log(B);
    ...
    /**
      * U面，蓝色
    */
   let U = new Array(this.state.cubeDimension)
   for(var i=0;i<this.state.cubeDimension;i++)
   {
     U[i] = new Array(this.state.cubeDimension)
   }
   for(let i =0; i<this.state.rubiksObject.length;i++)
    {
      if(this.state.cubes[i].position.z === this.state.cubeDimension-1)
      {
         var x = this.state.cubes[i].position.x;
         var y = this.state.cubeDimension-1-this.state.cubes[i].position.y;
        //  console.log("pos x: ", x, "y: ", y, "z: ", this.state.cubes[i].position.z);
        //  console.log("color: ", this.colorJudge(this.state.cubes[i].material[4].color));
         
         U[y][x] = this.colorJudge(this.state.cubes[i].material[4].color);
      }
    }
    // console.log(U);
    ...
    /**
      * D面，绿色
    */
   let D = new Array(this.state.cubeDimension)
   for(var i=0;i<this.state.cubeDimension;i++)
   {
     D[i] = new Array(this.state.cubeDimension)
   }
   for(let i =0; i<this.state.rubiksObject.length;i++)
    {
      if(this.state.cubes[i].position.z === 0)
      {
         var x = this.state.cubes[i].position.x;
         var y = this.state.cubes[i].position.y;
        //  console.log("pos x: ", x, "y: ", y, "z: ", this.state.cubes[i].position.z);
        //  console.log("color: ", this.colorJudge(this.state.cubes[i].material[5].color));
         
         D[y][x] = this.colorJudge(this.state.cubes[i].material[5].color);
      }
    }
    // console.log(D);
    ...
    /**
      * R面，红色
    */
   let R = new Array(this.state.cubeDimension)
   for(var i=0;i<this.state.cubeDimension;i++)
   {
     R[i] = new Array(this.state.cubeDimension)
   }
   for(let i =0; i<this.state.rubiksObject.length;i++)
    {
      if(this.state.cubes[i].position.x === this.state.cubeDimension-1)
      {
         var y = this.state.cubes[i].position.y;
         var z = this.state.cubeDimension-1-this.state.cubes[i].position.z;
        //  console.log("pos x: ", this.state.cubes[i].position.z, "y: ", y, "z: ", z);
        //  console.log("color: ", this.colorJudge(this.state.cubes[i].material[0].color));
         
         R[z][y] = this.colorJudge(this.state.cubes[i].material[0].color);
      }
    }
    // console.log(R);    
    ...
    /**
      * L面，橙色
    */
   let L = new Array(this.state.cubeDimension)
   for(var i=0;i<this.state.cubeDimension;i++)
   {
     L[i] = new Array(this.state.cubeDimension)
   }
   for(let i =0; i<this.state.rubiksObject.length;i++)
    {
      if(this.state.cubes[i].position.x === 0)
      {
         var y = this.state.cubeDimension-1-this.state.cubes[i].position.y;
         var z = this.state.cubeDimension-1-this.state.cubes[i].position.z;
        //  console.log("pos x: ", this.state.cubes[i].position.z, "y: ", y, "z: ", z);
        //  console.log("color: ", this.colorJudge(this.state.cubes[i].material[1].color));
    ...
    for(var i = 0; i< group.length;i++)
    {
      ...
      for(var j = 0;j<group[i].length;j++)
      {
        ...
        {
          var charOfFace;
          if(matrix[j][k] === 'White') charOfFace='F';
          else if(matrix[j][k] === 'Yellow') charOfFace = 'B';
          else if(matrix[j][k] === 'Blue') charOfFace = 'U';
          else if(matrix[j][k] === 'Green') charOfFace = 'D';
          else if(matrix[j][k] === 'Red') charOfFace = 'R';
          else if(matrix[j][k] === 'Orange') charOfFace = 'L';
          else console.log("Should never get here.");
          cubeToSeq+=charOfFace;
        }
      }
    }
  ...
  colorJudge = (color) => 
  {
    ...
    let colorGroup = [White, Red, Yellow, Green, Blue, Orange]
    let colorNameGroup = ['White', 'Red', 'Yellow', 'Green', 'Blue', 'Orange']
    let i=0
    for(i;i<colorGroup.length;i++)
    {
      if(colorGroup[i].equals(color))
      {
        return(colorNameGroup[i])
  ...
  reloadTurnedPieces = (pos) => {
    ...
    // console.log(this.state.rubiksObject.length)
    for(let i = 0; i<this.state.rubiksObject.length;i++){
  ...
  componentDidMount() {
    ...
    // let camera = new THREE.OrthographicCamera(window.innerWidth/-10, window.innerWidth/10, window.innerHeight/10, window.innerHeight/-10, .1, 1000);
